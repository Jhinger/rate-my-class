...
export async function POST(request: NextRequest, { params }: IClassProps) {
	...
	if (!session) {
		return NextResponse.json({
			error: "Not authenticated - you must sign in to post a rating.",
			status: 401,
		});
	}
	...
	if (userHasCommented && session.user.role === "USER") {
		return NextResponse.json({
			error: "Forbidden - You have already left a rating for this class.",
			status: 403,
		});
	}
	...
	try {
		...
		return NextResponse.json({
			error: "Server Error -  Could not find class and/or department",
			status: 500,
		});
	}
	...
	try {
		...
		return NextResponse.json({
			error: "Server Error - Failed to post rating.",
			status: 500,
		});
	}
