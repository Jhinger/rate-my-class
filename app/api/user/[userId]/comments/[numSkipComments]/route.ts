...
export async function POST(request: NextRequest, { params }: IUserCommentsParms) {
    const { numSkipComments, userId } = params;

    const session = await getServerSession(authOptions);
    if (!session || session.user.id !== userId) {
        return NextResponse.json({ error: "Unauthenticated - Must sign in order to fetch comments", status: 401 });
    }

    if (session.user.id !== userId) {
        return NextResponse.json({ error: "Forbidden - cannot fetch comments of another user.", status: 403 });
    }
