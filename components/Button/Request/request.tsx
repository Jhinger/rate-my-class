...
const Request = () => {
    ...
    return (
        <div className="flex flex-col justify-center items-center absolute top-0" onClick={updateFormVisibility}>
            <button id="request" className="font-semibold text-sm bg-primary rounded-md px-24 py-4 hover:bg-tertiary duration-300 w-max">
                REQUEST A SCHOOL / CLASS
            </button>
            {isOpen && 
                <form onClick={stopPropagation} className="flex flex-col bg-primary p-2 m-2 w-full rounded-md">
                    <label htmlFor="request-input" className="m-2 text-xs font-normal">Enter a school and optionally a class:</label>
                    <input 
                        autoFocus
                        id="request-input" 
                        className="rounded-sm p-2 m-2 ring-2"
                        type="text" 
